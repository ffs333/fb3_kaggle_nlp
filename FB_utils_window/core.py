...
def valid_fn(cfg, valid_loader, model, epoch, criterion, device, curclass=None):
    ...
    for step, (inputs, labels, length_) in pbar:

        full_len = labels.size(0)
        full_outputs = []
        for par in range(0, full_len, cfg.valid_bs):
            cur_inputs = dict()
            for k, v in inputs.items():
                cur_inputs[k] = v[par:min(full_len, par+cfg.valid_bs)]

            cur_labels = labels[par:min(full_len, par+cfg.valid_bs)]

            for k, v in cur_inputs.items():
                cur_inputs[k] = v.to(device)

            cur_labels = cur_labels.to(device)
            batch_size = cur_labels.size(0)

            with torch.no_grad():
                y_preds = model(cur_inputs)
                loss = criterion(y_preds, cur_labels)

            if cfg.gradient_accumulation_steps > 1:
                loss = loss / cfg.gradient_accumulation_steps
            losses.update(loss.item(), batch_size)

            full_outputs.append(y_preds.to('cpu').numpy())
        ...
        full_outputs = np.concatenate(full_outputs)
        start_idx = 0
        for val in length_:
            if cfg.use_mean_valid:
                cur_pred = np.mean(full_outputs[start_idx:start_idx+val], axis=0)
            else:
                cur_pred = full_outputs[start_idx:start_idx + val]
                if len(cur_pred) > 1:
                    wgts = np.concatenate([[0.5], [0.5 / (len(cur_pred) - 1) for _ in range(len(cur_pred) - 1)]])
                    cur_pred = np.sum(cur_pred * wgts.reshape(-1, 1), axis=0)
                else:
                    cur_pred = cur_pred[0]
